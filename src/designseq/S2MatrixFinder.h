...
private:
    ...
    string findMatrixFile(BackboneSitesPair& bsPair, int saiIndex);
    int findSaiIndex(BackboneSitesPair& bsPair);
    map<int,double> findSaiIndexList(BackboneSitesPair& bsPair);
    int findRPOIndex(BackboneSitesPair& bsPair);
    map<int,double> findRPOIndexList(BackboneSitesPair& bsPair);
...
public:
    ...
    void getSMNearestPair(BackboneSitesPair* rp, AAScoreMatrix* outputSM);

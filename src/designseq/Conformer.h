...
public:
    ...
    vector<PolarAtom*> bbPolarList;
    vector<PolarAtom*> scPolarList;
    ...
    Conformer(Rotamer* rot, BackBoneSite* bs, BackBoneSite* bsPre, AtomLib* atLib){
        this->triName = rot->triName;
        int n = rot->coordList.size();
        LocalFrame cs = getBackboneSiteLocalFrame(*bs);
        string s;
    ...
    void addPolarAtoms();
    void addPolarAtoms(BackBoneSite* bsPre);
    virtual ~Conformer();
};
...
class ConformerGroup{
    ...
    ConformerGroup(RotamerGroup* group, BackBoneSite* bs, BackBoneSite* bsPre, AtomLib* atLib){
        int n = group->rotNum;
        for(int i=0;i<n;i++){
            confList.push_back(new Conformer(group->rotList.at(i), bs, bsPre, atLib));
        }
        this->rotNum = n;
    }

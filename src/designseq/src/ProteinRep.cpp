...
NSPproteinrep::BackBoneSite Residue::getBackboneSite(){
    ...
    bs.resid = std::stoi(this->resID); // WARNING: NOT secure
...
Rotamer* Residue::natRotamer(AtomLib* atLib)
{
    ...
    {
        cout << "sidechain not complete: " << this->resID << endl;
        return NULL;
    }
    if(triName == "GLY") {
        Rotamer* r = new Rotamer("GLY-0", "GLY");
        return r;
    }
    LocalFrame& cs = getCoordSystem();
    Rotamer* r = new Rotamer(triName+"-0", triName);

    vector<string>* names = atLib->getAminoAcidSidechainAtomNames(triName);
    for(int i=0;i<names->size();i++)
    {
        Atom* a = getAtom(names->at(i));
        r->addAtom(a->getName(), cs.global2localcrd(a->getCoord()));
    }
    r->updateLawOfRing();
    return r;
}
...
vector<pair<int,double>> PhipsiLib::neighborPhipsiIndexList(const Phipsi* pp) const
{
    ...
    for(int i=0;i<n;i++){
        ...
        result.push_back(p);
    //    printf("%3d %6.4f",indexList[i], distList[i]);
    }
    //cout << endl;
    return result;
}
...
BackboneSitesPair::BackboneSitesPair(BackBoneSite* siteA, BackBoneSite* siteB){
    ...
    int sep = this->siteB->resseq - this->siteA->resseq;
    if(sep > 5) sep = 5;
    if(siteA->chainid != siteB->chainid) sep = 5;
    this->seqSep = sep;

...
RotamerLib::RotamerLib(const string& rotLibType)
{
    ...
    ifstream file;
    string rotPath = NSPdataio::datapath()+"abacus2/"+"rotLib/";
...
float RotamerLib::getRotamerEnergy(string& rotName, Phipsi* pp){

    int ppType = this->ppLib.phipsiToIndex(pp);
    return rotEnergyMap[rotName]->energyList[ppType];
    /*
    RotamerEnergyBBDep* re = rotEnergyMap[rotName];
    vector<pair<int,double>> neighborList = ppLib.neighborPhipsiIndexList(pp);
    int n = neighborList.size();
    double wtSum = 0;
    for(int i=0;i<n;i++){
        wtSum += pow(neighborList[i].second, -2);
    }
    double e = 0;
    double wt;
    for(int i=0;i<n;i++){
        wt = pow(neighborList[i].second, -2)/wtSum;
        e += re->energyList[neighborList[i].first]*wt;
    }
    */
}

float RotamerLib::getNearestRotamerEnergy(Rotamer* rot, int ppType){

    if(rot->triName == "XXX")
        return 0;
    ResName rn;
    int aaType = rn.triToInt(rot->triName);
    RotamerGroup* gp = this->aaRotGroups[aaType];
    if(gp->rotNum == 0)
        return 0;
    double minRMS = 99999.0;
    string minRotName = "UNK";

    for(int i=0;i<gp->rotNum;i++){
        double rms = rot->rms(gp->rotList[i]);
        if(rms < minRMS){
            minRMS = rms;
            minRotName = gp->rotList[i]->rotName;
        }
    }
    return getRotamerEnergy(minRotName, ppType);
}

...
struct XYZ {
	...
	double squaredDistance(const XYZ& other) const {
		return (x_-other.x_)*(x_-other.x_)+(y_-other.y_)*(y_-other.y_)+(z_-other.z_)*(z_-other.z_);
	}

	double distance(const XYZ& other) const{
		return std::sqrt((x_-other.x_)*(x_-other.x_)+(y_-other.y_)*(y_-other.y_)+(z_-other.z_)*(z_-other.z_));
	}
...
inline XYZ operator ~(const XYZ& p){
     ...
     double len = std::sqrt(p.x_*p.x_+p.y_*p.y_+p.z_*p.z_);
     if(len == 0){
